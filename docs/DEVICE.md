...
The Pi 3 camera runs at 15 FPS while the display renders the newest available frame at 6 FPS. The framebuffer only writes changed scanlines, keeping static header and control rows off the SPI bus. The display link runs at a moderate requested clock of 20 MHz, leaving substantially more signal margin than the unstable 24–32 MHz settings while allowing each live-image update to finish before the next one begins.
